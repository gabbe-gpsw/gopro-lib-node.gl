...
void main()
{
    vec4 position = ngl_position + vec4(data.positions[ngl_vertex_index], 0.0);
...
def compute_particules(cfg):
    ...
    idata = ngl.Block(layout='std430')
    idata.add_fields(
        ngl.BufferVec3(data=positions, label='positions'),
        ngl.BufferVec2(data=velocities, label='velocities'),
    )
    odata = ngl.Block(layout='std430')
    odata.add_fields(
        ngl.BufferVec3(count=nb_particules, label='positions'),
    )
    ...
    compute.update_resources(time=time, duration=duration, idata=idata, odata=odata)
    ...
    render.update_vertex_resources(data=odata)
...
void main()
{
    ...
    ivec2 size = ngl_imgsz(source);
    if (x < uint(size.x) && y < uint(size.y)) {
        vec4 color = ngl_img2d(source, ivec2(x, y));
