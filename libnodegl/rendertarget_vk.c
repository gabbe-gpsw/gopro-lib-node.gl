...
int ngli_rendertarget_init(struct rendertarget *s, struct ngl_ctx *ctx, const struct rendertarget_params *params)
{
    ...
    for (int i = 0; i < params->nb_colors; i++) {
        const struct attachment *attachment = &params->colors[i];
        const struct texture *texture = attachment->attachment;
        LOG(ERROR, "attachment_layer=%d %p", attachment->attachment_layer, texture->image);
        ...
        if (attachment->resolve_target) {
            const struct texture *texture = attachment->resolve_target;
    ...
    if (texture) {
        ...
        if (attachment->resolve_target) {
            struct texture *resolve_target = attachment->resolve_target;
            VkImageViewCreateInfo view_info = {
                .sType = VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO,
                .image = resolve_target->image,
                .viewType = VK_IMAGE_VIEW_TYPE_2D,
                .format = resolve_target->format,
                .subresourceRange.aspectMask = get_vk_image_aspect_flags(resolve_target->format),
                .subresourceRange.baseMipLevel = 0,
                .subresourceRange.levelCount = 1,
                .subresourceRange.baseArrayLayer = 0,
                .subresourceRange.layerCount = 1,
    ...
    LOG(ERROR, "attachmentCount=%d", s->nb_attachments);
