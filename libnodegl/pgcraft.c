...
static int inject_texture_info(struct pgcraft *s, int stage, struct pgcraft_texture_info *info)
{
    for (int i = 0; i < NGLI_INFO_FIELD_NB; i++) {
        ...
        if (ngli_type_is_sampler_or_image(field->type)) {
            ...
            if (s->next_tex_bind)
                pl_texture.binding = s->next_tex_bind[0]++; // XXX
                        ...
                        const struct pgcraft_named_block *named_block)
{
    ...
    int *bindings = block->type == NGLI_TYPE_STORAGE_BUFFER ? s->next_ssbo_bind : s->next_ubo_bind;
    if (bindings) {
        pl_buffer.binding = bindings[0]++; // XXX: bindings are common accross stages!
...
static void setup_glsl_info(struct pgcraft *s, const struct vkcontext *vk)
{
    ...
    s->next_ssbo_bind = s->bindings;
    s->next_ubo_bind  = s->bindings;
    s->next_tex_bind  = s->bindings;
...
static void setup_glsl_info(struct pgcraft *s, const struct glcontext *gl)
{
    ...
    /*
     * TODO: add ubo bindings (need a separate binding list maybe?) when
     * program probing code binding force is removed
     */
    s->next_ssbo_bind = s->bindings;
    s->next_ubo_bind = NULL;
    s->next_tex_bind = NULL;
