...
def _block(w, h, program, corner=None, **uniforms):
    ...
    block_render.update_fragment_resources(**uniforms)
    return block_render
...
def _get_easing_node(cfg, easing, curve_zoom, color_program, nb_points=128):
    ...
    curve.update_fragment_resources(color=ucolor)
    ...
    cursor.update_fragment_resources(color=ucolor)
    ...
    hline.update_fragment_resources(color=line_ucolor)
    ...
    vline.update_fragment_resources(color=line_ucolor)
...
def easings(cfg, easing_id='*'):
    ...
    color_program.update_vert2frag_vars(var_uvcoord=ngl.IOVariable('vec2'))
